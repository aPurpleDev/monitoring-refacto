...
const selectDBSizes = async () => {
    ...
    const dbLogs = [];
    const dbSizes = await db.sequelize.query(`select datname, pg_size_pretty(pg_database_size(datname)) from pg_database order by pg_database_size(datname);`);
...
const selectOSTablePercentUsed = async () => {
    ...
    const dbLogs = [];
    const dbSizes = await db.sequelize.query(`SELECT relname, 100 * idx_scan / (seq_scan + idx_scan) percent_of_times_index_used, n_live_tup rows_in_table FROM pg_stat_user_tables ORDER BY n_live_tup DESC;`);
...
const getUserConnections = async () => {
    ...
    const dbLogs = [];
    const dbSizes = await db.sequelize.query(`select client_addr, usename, datname, count(*) from pg_stat_activity group by 1,2,3 order by 4 desc;`);
    for (let entry of dbSizes) {
        dbLogs.push(entry);
    }
    db_JSONlogs.data = dbLogs[0];
